...
namespace osu.Game.Rulesets.UMania.Beatmaps.Patterns.Legacy
{
    /// <summary>
    ...
    internal class SliderPatternGenerator : LegacyPatternGenerator
    {
        ...
        public SliderPatternGenerator(LegacyRandom random, HitObject hitObject, IBeatmap beatmap, int totalColumns,
            ...
            : base(random, hitObject, beatmap, previousPattern, totalColumns)
        {
            ...
            if (hitObject is IHasSliderVelocity hasSliderVelocity)
            {
                ...
                double bpmMultiplier = sliderVelocityAsBeatLength < 0 ? Math.Clamp((float)-sliderVelocityAsBeatLength, 10, 10000) / 100.0 : 1;
                beatLength =
                    timingPoint.BeatLength * bpmMultiplier;
            }
            else
            ...
            EndTime = (int)Math.Floor(StartTime +
                                      distance * beatLength * SpanCount * 0.01 / beatmap.Difficulty.SliderMultiplier);
        ...
        private Pattern generateNRandomNotes(int startTime, double p2, double p3, double p4)
        {
            ...
            static bool isDoubleSample(HitSampleInfo sample) =>
            ...
            canGenerateTwoNotes &=
                HitObject.Samples.Any(isDoubleSample) || sampleInfoListAt(StartTime).Any(isDoubleSample);
        ...
        private Pattern generateHoldAndNormalNotes(int startTime)
        {
            ...
            bool ignoreHead = !sampleInfoListAt(startTime).Any(s =>
                s.Name == HitSampleInfo.HIT_WHISTLE || s.Name == HitSampleInfo.HIT_FINISH ||
            ...
            for (int i = 0; i <= SpanCount; i++)
            {
                if (!(ignoreHead && startTime == StartTime))
                {
                    ...
                    {
                        nextColumn = FindAvailableColumn(nextColumn, validation: c => c != holdColumn,
                            patterns: rowPattern);
