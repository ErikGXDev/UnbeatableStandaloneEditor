...
using osu.Game.Rulesets.UMania.Beatmaps.Patterns;
using osu.Game.Rulesets.UMania.Beatmaps.Patterns.Legacy;
...
namespace osu.Game.Rulesets.UMania.Beatmaps
{
    public class ManiaBeatmapConverter : BeatmapConverter<ManiaHitObject>
    {
        ...
        private ManiaBeatmapConverter(IBeatmap? beatmap, LegacyBeatmapConversionDifficultyInfo difficulty,
            Ruleset ruleset)
            : base(beatmap!, ruleset)
        {
            IsForCurrentRuleset = difficulty.SourceRuleset.Equals(ruleset.RulesetInfo);
            Random = new LegacyRandom((int)MathF.Round(difficulty.DrainRate + difficulty.CircleSize) * 20 +
            ...
            */
            ...
            /*static int getColumnCount(LegacyBeatmapConversionDifficultyInfo difficulty)
            {
                ...
                if (difficulty.SourceRuleset.ShortName == UbManiaRuleset.SHORT_NAME)
        ...
        public static int GetColumnCount(LegacyBeatmapConversionDifficultyInfo difficulty,
            IReadOnlyCollection<Mod>? mods = null)
        {
            var converter = new ManiaBeatmapConverter(null, difficulty, new UManiaRuleset());
            ...
            CancellationToken cancellationToken)
        {
            ...
            switch (legacyType)
            {
                case LegacyHitObjectType.Circle:
                    ...
                    {
                        conversion =
                            new PassThroughPatternGenerator(Random, original, beatmap, TotalColumns, lastPattern);
                    ...
                    else
                    {
                        ...
                        conversion = new HitCirclePatternGenerator(Random, original, beatmap, TotalColumns, lastPattern,
                            lastTime, lastPosition, density, lastStair);
                        recordNote(startTime, position);
                    }
                ...
                case LegacyHitObjectType.Slider:
                    ...
                    {
                        var generator =
                            new SliderPatternGenerator(Random, original, beatmap, TotalColumns, lastPattern);
