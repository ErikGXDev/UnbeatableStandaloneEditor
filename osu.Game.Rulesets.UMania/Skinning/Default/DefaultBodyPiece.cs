...
namespace osu.Game.Rulesets.UMania.Skinning.Default
{
    /// <summary>
        ...
        private partial class ForegroundPiece : CompositeDrawable
        {
            ...
            private void load()
            {
                ...
                IsHitting.BindValueChanged(
                    _ => onAccentChanged(new ValueChangedEvent<Color4>(AccentColour.Value, AccentColour.Value)), true);
            }
                ...
                if (IsHitting.Value)
                {
                    ...
                    using (foregroundBuffer.BeginDelayedSequence(synchronisedOffset))
                        foregroundBuffer.FadeColour(accent.NewValue.Lighten(0.2f), animation_length).Then()
                                        .FadeColour(foregroundBuffer.Colour, animation_length).Loop();
                }
